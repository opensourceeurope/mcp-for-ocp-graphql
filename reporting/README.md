...
| `export_least_active_collectives.py` | The 10 least active collectives of a host: oldest last financial operation first (never-active ones on top), with creation date and last activity. Archived ones are marked; the host itself is excluded. Public data — no token needed. | `uv run export_least_active_collectives.py`<br>Options: `--slug <host>`, `--top <n>`, `--format csv`/`pdf`, `--out <path>`.<br><br>**Output:** `output/<host-slug>-least-active.{md,csv,pdf}`, default `…/europe-least-active.md` |
