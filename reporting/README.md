...
| `export_never_active_collectives.py` | Every collective of a host that never had any activity — not a single financial operation or published update, including in its events/projects — with creation date, oldest first. Archived ones are marked; the host itself is excluded. Public data — no token needed. | `uv run export_never_active_collectives.py`<br>Options: `--slug <host>`, `--format csv`/`pdf`, `--out <path>`.<br><br>**Output:** `output/<host-slug>-never-active.{md,csv,pdf}`, default `…/europe-never-active.md` |
