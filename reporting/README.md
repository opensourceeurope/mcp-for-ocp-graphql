...
| `export_least_active_collectives.py` | The 10 least active collectives of a host: oldest last activity first (never-active ones on top), with creation date and last activity. Activity = financial operations or published updates, including those of the collective's events/projects. Archived ones are marked; the host itself is excluded. Public data — no token needed. | `uv run export_least_active_collectives.py`<br>Options: `--slug <host>`, `--top <n>`, `--format csv`/`pdf`, `--out <path>`.<br><br>**Output:** `output/<host-slug>-least-active.{md,csv,pdf}`, default `…/europe-least-active.md` |
| `export_top_collectives.py` | Four top-10 rankings of a host's collectives over a period (default: current year → now): most money received, most distinct donors, most money paid out, most distinct payees. Events/projects roll up into their parent collective. Public data — no token needed. | `uv run export_top_collectives.py`<br>Options: `--slug <host>`, `--date-from`/`--date-to YYYY-MM-DD`, `--top <n>`, `--format csv`/`pdf`, `--out <path>`.<br><br>**Output:** `output/<host-slug>-top-collectives.{md,csv,pdf}`, default `…/europe-top-collectives.md` |
