...
- `cherry-studio/opencollective-analyst/SKILL.md` — the Cherry Studio skill, generated by
  `scripts/build_cherry_skill.py` by merging the plugin's agent
  (`plugins/oc-platform-api/agents/opencollective-analyst.md`) + querying skill
  (`plugins/oc-platform-api/skills/querying-opencollective-graphql/SKILL.md`). To change it,
  edit one of those two sources and regenerate: `uv run python scripts/build_cherry_skill.py`.
  `ci.yml` re-runs the generator and `git diff --exit-code`s the output, so a stale commit
  fails CI; the branch-guard hook denies direct edits. On release, `release.yml` zips this
  dir and uploads it to the GitHub Release as `opencollective-cherry-skill.zip`.
