...
The Open Collective API can return **personal data** — `email`/`emails`, `phoneNumber`, `address`, `legalName`, and anything inside `payoutMethod`, `paymentMethod`, or `location` on Individual accounts. Selecting any such field pulls it into the model's processing context — for a hosted model that means transmission to the provider's servers; for a local model it stays on the user's machine. Writing it to a file, commit, log, or message is a **further** disclosure. None of this can be undone after the fact — the only control point is **before** the field is selected.

- **By default, do not select PII fields** in any `graphql_query` / MCP query. Don't slip them into a wider selection, and don't have a subagent fetch them.
- **When the user asks for PII (or you'd recommend fetching it):** tell them plainly what will happen first — the data enters the model's context (the provider's servers under a hosted model, their machine under a local one), and anywhere you then write it is a further disclosure. If you don't know which kind of model you're running under, say so. Once they confirm with that awareness, do what they asked — it's their data and their call.
- **Preferred alternative:** if they'd rather keep PII off the model entirely, give them the `curl` recipe to run in their own terminal (output stays local) — see [`docs/using-with-ai-safely.md`](docs/using-with-ai-safely.md) › "When you genuinely need the PII".

This is the agent-facing form of the rule in [`docs/using-with-ai-safely.md`](docs/using-with-ai-safely.md); the query-work version lives in the [`querying-opencollective-graphql` skill](plugins/oc-platform-api/skills/querying-opencollective-graphql/SKILL.md).
