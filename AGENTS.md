...
- Pick the type by what ships: changes under `plugins/oc-platform-api/` (including skill prose) only reach users via a plugin release → `feat:`/`fix:`, never `docs:`/`chore:`; `reporting/` scripts are side artifacts → `chore:` (no release).
