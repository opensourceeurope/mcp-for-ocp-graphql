...
- [`corpus-refresh.yml`](.github/workflows/corpus-refresh.yml) runs the whole pipeline on Linux CI: `schema_fetch` → build the curated query-field map (`schema_ref --queries-only` → `.opencrane/sources/local/schema-queries.md`) → `opencrane fetch` (clones `opencollective/graphql-docs-v2` into `.opencrane/sources/`) → `opencrane llms` → `opencrane chunk` → `opencrane embed` → **`python -m …indexer`**.
